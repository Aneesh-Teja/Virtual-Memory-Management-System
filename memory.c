...
void setup_page_table(pcb_t * p)
{
    ...
    else
    {
        ...
        // -------------------------------
        // 4. Allocate ONE Code/Data Page
        // -------------------------------
        page_index = page_alloc(FALSE);   // not pinned (can be swapped)

        init_ptab_entry(code_ptab,
                        PROCESS_START,
                        (uint32_t)page_addr(page_index),
                        mode);
        ...
        // -------------------------------
        // 6. Map Kernel Memory (shared)
        // -------------------------------
        insert_ptab_dir(p->page_directory,
                        kernel_ptabs[0],
                        (uint32_t)kernel_ptabs[0],
                        mode);
    }
